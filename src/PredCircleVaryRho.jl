...
h = 1 / NTrain
xTrain = h : h : 1 
xTrain = 2 * vcat(0.5 * sin.(2 * π * xTrain)', 
              cos.(2 * π * xTrain)')
xTest = 0.05 * randn(2,NTest) 
x = hcat(xTrain, xTest)
...
for (ρInd, ρ)  in enumerate(ρList)
  ...
  global stdRMSENoPred
    ...
    # Creating reference solution, ignoring the testing part
    y = vcat(chol_Θ_train.L * randn(NTrain), zeros(NTest))
...
save("./out/jld/IncludePredCircleVaryRho$maternOrder.jld", 
