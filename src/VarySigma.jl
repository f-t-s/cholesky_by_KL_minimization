...
l = 1.1
σ = vcat(zeros(NTest), fill(1.0, NTrain))
# σ = vcat(zeros(NTest), 0.1 * exp.(randn(NTrain)) .+ 1e-4)
...
A22 = L22 * L22' + Diagonal(1 ./ max.(σ[Block(2)], 1e-30))
# U22 = sparse(triu(A22))
U22 = squareSparse(L22)+ Diagonal(1 ./ max.(σ[Block(2)], 1e-30))
